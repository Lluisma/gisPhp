...
/**
 * @modified Lluís Martí i Garro <lluis.manies.cat>
 * Includes buffer option [ lines 60.64.65 | 91-101 | 106-120 ] 
 */
...
class gisPhp_ConvexHull 
{
     ...
     * @param array $points 
     */
    public function __construct( array $pofloats, $offset = 0, $precision = 4) 
    {
...
// FIXED - LLUÍS MARTÍ I GARRO - 01.03.2012 -------------------------------------------------------
        // * Stablishes offset value, draws a circle in every point of the hull
        // * and recalculates their hull
        if ($this->offset != 0) { 
          $this->inputPoints = $this->getOffsetPoints();
          // * Resets values and recalls getHullPoints (with no offset) 
          $this->offset = 0;
          $this->hullPoints = null;          
          $this->hullPoints  = $this->getHullPoints();
        } 
// FIXED - LLUÍS MARTÍ I GARRO - 01.03.2012 -------------------------------------------------------
...
// FIXED - LLUÍS MARTÍ I GARRO - 01.03.2012 -------------------------------------------------------
    protected function getOffsetPoints() {
      $offsetPoints = array();      
      foreach ( $this->hullPoints as $punt) {
      	$x = $punt[0];
      	$y = $punt[1];
        for ($i = 0;  $i < 2*pi(); $i += ( pi()/$this->precision)) {
          $x2 = $x -(sin($i) * $this->offset * $this->offset);
          $y2 = $y -(cos($i) * $this->offset * $this->offset);
          $offsetPoints[] = array($x2, $y2);
        }
      }
      return $offsetPoints;
    }
